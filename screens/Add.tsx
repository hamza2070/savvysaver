...
export const Add = () => {
  ...
  const [category, setCategory] = React.useState<Category>(categories[0] ?? {} as Category);

  const onCategoriesChange = (newCategories : any) => {
  ...
  const submitExpense = () => {
    let team =  realm.objects('Expense');
    console.log(team);
    console.log(amount, recurrence, date, note, category)
  ...
  return (
    <>
      <KeyboardAvoidingView
        ...
        style={[{ padding: 16, flex: 1, alignItems: 'center' },stylesGlobal.background]}
      >
        <View
          style={{
            borderRadius: 11,
